...
| 8 | **Who emits Z1 (suppression) and Z2 (deferred-expiry) to M14?** (added 2026-09-02). Your A6 produces `EngagementDecision` with a `SuppressionReason`, and A7 will expire deferred entries. M14 defines and counts `OutcomeKind.suppressed` / `deferred_expired` but **nothing emits them**, so suppression rate currently reads as no-data. The adapter is small — it just needs a home and an agreed shape. | POA/14 task 1's remaining half; the suppression-rate metric | A metric that silently reads 0/no-data forever, which nobody notices until someone asks why triggers look unsuppressed |
...
| A3 Event Capture SDK | Prasad | in progress | 2026-09-02 core done + tested: Python reference/server-side capture SDK (CaptureClient) emitting schema-valid Events for the 8 signals to A4's /v1/events:batch; session correlation, consent gating, capped offline buffer + retry (no loss on outage), SDK→A4 contract + idempotency tested. Deferred: client-side JS SDK + browser detectors (POA/01 §12). **Completes Track A (A1–A8).** |
...
| A8 Human Handoff | Prasad | in progress | 2026-09-02 core done + tested (kept THIN around RoutingRule dicts): HandoffRequest contract, first-match routing over M13 dicts + catch-all default, context packaging (refs+summary, no inlined PII), QueueAdapter with retry + no-agent/after-hours fallback + dead-letter (never drops), lifecycle ledger (routed→accepted→resolved) for M14. Cross-track: HandoffRequest added to generator/models.py, reconcile with M12/B6. Deferred: concrete support-platform adapter, M12→M04 intake wiring (POA/07 §11) |
